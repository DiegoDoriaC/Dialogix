...
--- Exportar cantidad de consultas especialidad
create procedure pr_listar_atencion_por_especialidad
	@FechaInicio datetime,
	@FechaFin datetime
as
begin
	select med.Especialidad, COUNT(*) as cantidad from CitaMedica con
	left join Medico med ON med.id = con.IdMedico
	where (@FechaInicio = '' or @FechaInicio <= con.FechaCita)
	AND (@FechaFin = '' or @FechaFin >= con.FechaCita)
	group by med.Especialidad order by cantidad desc
end
go